...
public class DBConfig extends DefaultDbConfig {
    ...
    SpringDataJdbcProperties botJdbcProperties() {
        return new SpringDataJdbcProperties();
    }

    @Bean
    @Qualifier("bot-db")
    public DataSource botDataSource(@Qualifier("bot-db") SpringDataJdbcProperties properties) {
        return hikariDataSource("db", properties);
    }

    @Bean
    @Qualifier("bot-db")
    JdbcTemplate botJdbcTemplate(@Qualifier("bot-db") DataSource dataSource) {
